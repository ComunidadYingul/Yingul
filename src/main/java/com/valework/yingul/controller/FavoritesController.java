...
public class FavoritesController {
    ...
    public String createFavorite(@PathVariable("itemId") Long itemId,@PathVariable("username") String username) {
		...
		if(yng_Item.getUser().getUsername()==yng_User.getUsername()) {
		    ...
		    /*try {
				smtpMailSender.send(yng_User.getEmail(), "Yingul favoritos", "Agregaste a favoritos "+yng_Item.getName()+" "+yng_Item.getDescription()+" para ver tus productos, servicios, inmuebles o vehiculos favoritos ingresa a: http://yingulportal-env.nirtpkkpjp.us-west-2.elasticbeanstalk.com/favorites");
